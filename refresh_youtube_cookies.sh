...
What to do before running this script again:
  ...
  3. Confirm Deno is installed (required for yt-dlp YouTube JS challenges):
       deno --version
     If that fails, install Deno and ensure it is on your PATH, then re-run.
...
# Install into place: tuberipper and _yt_dlp_common_options() expect this path.
mv -f "$COOKIES_NEW" "$COOKIES_FILE"
echo "Done. Cookies are in the project cookies folder at:"
echo "  $COOKIES_FILE"
