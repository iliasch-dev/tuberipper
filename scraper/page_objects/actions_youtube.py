...
class ActionYoutube:
    ...
    def __init__(self, logger, page, db_conn):
        ...
        self.logger = logger
        self.home_page = HomePage(logger, page, db_conn)
    ...
    def scrap_video_audio(self):
        ...
        if channels is not None:
            for channel in channels:
                try:
                    self.scrap_single_channel(channel)
                    time.sleep(5)
    ...
    def scrap_one_channel(self, channel):
        """Manual-trigger entry point: sets up the browser session (navigate + cookie
        consent) and scrapes a single channel, honoring its scrap_streams/scrap_videos flags."""
        self.logger.info("Looking for videos to audio-scrap")
        self.home_page.navigate_to_youtube()
        self.home_page.click_reject_button()
        try:
    ...
    def scrap_single_channel(self, channel):
                ...
                if not video_id:
                    ...
                    if result is not None and all(value not in [None, "", [], {}, set()] for value in result.values()):
                        ...
                        db_client.insert_rip_record(
                            self.db_conn, channel['channel'], result['video_title'],
                            result['video_duration'], Format.AUDIO.value, "STREAM",
        ...
        if channel['scrap_videos']:
            ...
            if not video_id:
                ...
                if result is not None and all(value not in [None, "", [], {}, set()] for value in result.values()):
                    ...
                    db_client.insert_rip_record(
                        self.db_conn, channel['channel'], result['video_title'],
                        result['video_duration'], Format.AUDIO.value, "VIDEO",
                        result['video_thumbnail_url'], video_id, result['audio_filename']
                    )
                    logging.info(f"Sending pushover: title={result['video_title']}, image={result['video_thumbnail_url']}")
                    utils.send_pushover_notification(
